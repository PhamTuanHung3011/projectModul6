...
public interface FriendRepo extends JpaRepository<Friend, Long> {
    ...
    @Query(nativeQuery = true,value = "select * from users join (select bb1.id from (select friend.user2_id as id from friend where friend.user1_id =:idUser1 and friend.status = 1 union select friend.user1_id as id1 from friend where friend.user2_id =:idUser2 and friend.status = 1) as bb1 join (select friend.user2_id as id3 from friend where friend.user1_id = idUser1 and friend.status = 1 union select friend.user1_id as id4 from friend where friend.user2_id =: idUser2 and friend.status = 1) as bb2 on bb1.id = bb2.id) as bbc on users.id = bbc.id")
